...
public class AStarAlgorithm {
	...
	public AStarAlgorithm(RoadNetwork graph, List<ListRequest> oblist) {
		this.graph = graph;
	...
	// 출발지(source node) 에서 목적지(target node) 까지의 최단 경로를 계산
	// 최단 경로의 비용을 반환
	// 참고: target node -1과 함께 호출되면 모든 노드까지 AStar가 실행됨
		...
		while (!distances.isEmpty()) {
			...
			for (Edge edge : edges) {
				...
				double heuristic = HaversineDistance.distance(graph.nodes.get(currentNodeId).latitude,
						...
						graph.nodes.get(targetNodeId).longitude);
